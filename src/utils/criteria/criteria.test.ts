...
describe('compareCriteriaByDefaultRowIdxFn(...)', () => {
  describe('ASC', () => {
    it('Already sorted criteria array', () => {
      expect([...criteriaSortedByDefaultRowIdx.ASC].sort(compareCriteriaByDefaultRowIdxFn(ASC))).toStrictEqual(
        criteriaSortedByDefaultRowIdx.ASC
      );
    });

    it('Unsorted criteria array', () => {
      expect([c1, c2, c3].sort(compareCriteriaByDefaultRowIdxFn(ASC))).toStrictEqual(criteriaSortedByDefaultRowIdx.ASC);
    });
  });

  describe('DESC', () => {
    it('Already sorted criteria array', () => {
      expect([...criteriaSortedByDefaultRowIdx.DESC].sort(compareCriteriaByDefaultRowIdxFn(DESC))).toStrictEqual(
        criteriaSortedByDefaultRowIdx.DESC
      );
    });

    it('Unsorted criteria array', () => {
      expect([c1, c2, c3].sort(compareCriteriaByDefaultRowIdxFn(DESC))).toStrictEqual(
        criteriaSortedByDefaultRowIdx.DESC
      );
    });
  });
...
describe('calculateCriteriaNormalizedWeight(...)', () => {
  const criteriaWithNormalizedWeights = calculateCriteriaNormalizedWeights(criteria);
  ...
  it('Ensure normalized weight total is 1', () => {
    expect(
      criteriaWithNormalizedWeights.reduce((total, criterion) => total + (criterion.normalizedWeight ?? 0), 0)
    ).toEqual(1);
  });
});
...
describe('getCriterionWeightRelativeToMax(...)', () => {
  ...
  it('Expect correct value', () => {
    expect(criteria.map(criterion => getCriterionWeightRelativeToMax(criterion.weight, maxWeight))).toEqual([
      100, // ** c1.weight = 2 (100%)
      50, //  ** c2.weight = 1 ( 50%)
      50 //  ** c3.weight = 1 ( 50%)
    ]);
